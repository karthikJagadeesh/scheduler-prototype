...
dayPilot.onTimeRangeSelected = args => {
    const name = prompt("Enter new event name", "Event")
    dayPilot.clearSelection()

    if (!name)
        return

    const e = new DayPilot.Event({start: args.start, end: args.end, id: DayPilot.guid(), resource: args.resource, text: name})

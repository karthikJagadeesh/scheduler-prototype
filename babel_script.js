...
var resources = dayPilot.resources = [{
    name: "Task 1",
    id: "task1",
    expanded: true,
    children: [{
...
dayPilot.contextMenu = new DayPilot.Menu({
    items: [{
        ...
        onclick: function onclick() {
            ...
            if (response) dayPilot.events.remove(this.source);
        }
    }, {
        text: "Copy",
        onclick: function onclick() {
...
dayPilot.contextMenuSelection = new DayPilot.Menu({
    items: [{
        text: "paste",
        onclick: function onclick() {
            if (!dayPilot.clipBoard) alert("Nothing copied to paste");

            var selection = this.source;
            var clipBoard = dayPilot.clipBoard;

            var duration = clipBoard.end().getTime() - clipBoard.start().getTime();
